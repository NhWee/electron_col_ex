...
# Standard spin-1-exchange shape (3/8)*[(1+cos^2 theta) + (8/3)*A_FB*cos(theta)].
# A_FB from standard lepton vector/axial couplings, treated here as
# energy-independent (real LEP A_FB flips sign below/above the peak from
# gamma/Z interference -- not modeled). Since the acceptance cut below is
# symmetric in cos(theta), A_FB has zero effect on the accepted count either
# way -- it's carried through anyway so generated events have real per-event
# angular info on hand for a future A_FB/sin2(theta_W) exercise, without
# building that exercise now.
SIN2_THETAW_EFF = 0.23155
V_LEPTON = -0.5 + 2 * SIN2_THETAW_EFF
A_LEPTON = -0.5
A_LEPTON_ASYM = 2 * V_LEPTON * A_LEPTON / (V_LEPTON**2 + A_LEPTON**2)
A_FB = 0.75 * A_LEPTON_ASYM**2
...
def angular_pdf(costheta: np.ndarray) -> np.ndarray:
    return 0.375 * ((1 + costheta**2) + (8 / 3) * A_FB * costheta)


def sample_costheta(n: int, rng: np.random.Generator) -> np.ndarray:
    ...
    while accepted.size < n:
        ...
        accepted = np.concatenate([accepted, candidates[u < angular_pdf(candidates)]])
    return accepted[:n]
...
def measure_acceptance(rng: np.random.Generator) -> tuple[float, float]:
    ...
    paper."""
    costheta = sample_costheta(EFFICIENCY_SAMPLE_SIZE, rng)
...
def generate_scan_point(sqrt_s: float, a_hat: float, a_hat_err: float, rng: np.random.Generator) -> dict:
    ...
    costheta = sample_costheta(int(n_gen), rng)
    n_selected = int(np.sum(np.abs(costheta) < ACCEPTANCE_COSTHETA_MAX))
